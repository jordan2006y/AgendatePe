...
fun NavigationWrapper(
    ...
    NavHost(
        ...
        // ANIMACIÓN GLOBAL: Deslizar izquierda/derecha
        ...
        composable(route = "profile/{email}/{password}", arguments = listOf(navArgument("email") { type = NavType.StringType }, navArgument("password") { type = NavType.StringType })) { entry ->
            ProfileScreen(auth = auth, emailRecibido = entry.arguments?.getString("email") ?: "", passwordRecibido = entry.arguments?.getString("password") ?: "", navigateToHome = { navHostController.navigate("home") { popUpTo("initialScreen") { inclusive = true } } })
        }
        ...
        // Animación especial para Publicar (aparece desde abajo)
